...
use yixuan_proto::PlayerGetTokenScRsp;
...
impl SecurityModule {
    pub fn gen_rand_key(
        ...
        let client_rand_key =
            yixuan_encryption::rsa::decrypt(&version.server_private_key, client_rand_key)
        ...
        rsp.server_rand_key = yixuan_encryption::rsa::encrypt(
            &version.client_public_key,
            &server_rand_key.to_le_bytes(),
        );

        rsp.sign = yixuan_encryption::rsa::sign(
